...
def edit(request, video_id):
    ...
    if hasattr(video, "track_listing"):
        action_url = reverse(
            "wagtailvideos_tracks:edit", args=(video.track_listing.pk,)
        )
    else:
    ...
    return render(
        ...
        "wagtailvideos/videos/edit.html",
        {
            ...
            "transcoder_enabled": not getattr(
    ...
    if request.POST:
        try:
            ...
            if form.is_valid():
                ...
                messages.error(
                    request, _("The video could not be created due to errors.")
                )
        finally:
...
def usage(request, video_id):
    ...
    return render(
        request, "wagtailvideos/videos/usage.html", {"video": video, "used_by": page}
    )
